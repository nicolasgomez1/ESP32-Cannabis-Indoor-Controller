...
#define HW080_MAX 2800      // I'm using 20k resistors between RETURN LINE & GND, so the max value never go up to 4095
...
enum g_Recommendations {
	TEMP_UP, TEMP_DOWN,
	HUM_UP, HUM_DOWN,
	OPTIMAL, THIS_COULD_GET_UGLY
};

const char* g_cRecommends[] = {
	"▲ Temperatura.", "▼ Temperatura.",
	"▲ Humedad.", "▼ Humedad.",
	"Óptimo.", "Fuera de Rango"
};
...
// - bUseSafeSDAccess: If true, wraps the operation inside SafeSDAccess for thread-safe access. If the SD is not initialized, the function exits without performing any action.
...
// Writes one setting per line, including Wi-Fi credentials, sampling interval, current profile, fan intervals, hysteresis values, flow rates, mixing pump duration, and irrigation reservoir level.
...
void LoadProfile(uint8_t nProfile) {
	SafeSDAccess([&]() {
		...
		if (pProfileFile) {
			...
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MIN PROFILE TEMPERATURE RANGE
			g_fTemperatureRanges[0] = atof(cBuffer);

			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MAX PROFILE TEMPERATURE RANGE
			g_fTemperatureRanges[1] = atof(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MIN PROFILE HUMIDITY RANGE
			g_fHumidityRanges[0] = atof(cBuffer);

			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MAX PROFILE HUMIDITY RANGE
			g_fHumidityRanges[1] = atof(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MIN PROFILE VPD RANGE
			g_fVPDRanges[0] = atof(cBuffer);

			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer));	// MAX PROFILE VPD RANGE
			g_fVPDRanges[1] = atof(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer)); // VPD CORRECTION PRIORITY
			g_nVPDCorrectionPriority = atoi(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer)); // IRRIGATION PULSE INTERVAL DIVIDER
			g_nPulseIntervalDivider = atoi(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer)); // PULSE INTERVAL AFTER LIGHTS TURN ON
			g_nPulseIntervalAfterStart = atoi(cBuffer);
			///////////////////////////////////////////////////
			ReadFromStream(pProfileFile, cBuffer, sizeof(cBuffer)); // PULSE INTERVAL BEFORE LIGHTS TURN OFF
			g_nPulseIntervalBeforeEnd = atoi(cBuffer);
...
void SaveProfile(uint8_t nProfile) {
	SafeSDAccess([&]() {
		...
		if (pProfileFile) {
			...
			pProfileFile.println(g_fTemperatureRanges[0]);
			pProfileFile.println(g_fTemperatureRanges[1]);

			pProfileFile.println(g_fHumidityRanges[0]);
			pProfileFile.println(g_fHumidityRanges[1]);

			pProfileFile.println(g_fVPDRanges[0]);
			pProfileFile.println(g_fVPDRanges[1]);

			pProfileFile.println(g_nVPDCorrectionPriority);

			pProfileFile.println(g_nPulseIntervalDivider);
			pProfileFile.println(g_nPulseIntervalAfterStart);
			pProfileFile.println(g_nPulseIntervalBeforeEnd);
...
void SendNotification(const char* cMessage) {
	...
	char cFinalUrl[256];
	snprintf(cFinalUrl, sizeof(cFinalUrl), "https://api.callmebot.com/whatsapp.php?phone=%s&text=%s&apikey=%s", g_cCALLMEBOT_PHONE, cEncodedMessage, g_cCALLMEBOT_API_KEY);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Computes the Vapor Pressure Deficit (VPD) in kPa using temperature (°C) and relative humidity (%).
...
// The level is calculated relative to the configured lower reference level.
...
// Supports generic numeric types via templating (e.g., uint8_t, uint16_t, int, float).
...
template <>
bool CheckNSetValue<float>(AsyncWebServerRequest* pRequest, const char* cArgumentName, float& pDestination, const __FlashStringHelper* flashMessageToReturn, String& strReturn) {
    if (pRequest->hasArg(cArgumentName)) {
        float fNewValue = pRequest->arg(cArgumentName).toFloat();

        if (fNewValue != pDestination) {
            pDestination = fNewValue;
            strReturn += F("Se actualizó ");
            strReturn += flashMessageToReturn;
            strReturn += F(".\r\n");
...
// Supports returning environmental data, system states, and dynamically generated HTML blocks for insertion into templates. Used with templating engines (e.g., AsyncWebServer's .setTemplateProcessor).
String HTMLProcessor(const String& var) {
	if (var == "MAXBRIGHT") {
	...
	} else if (var == "VPDCORRECTIONPRIORITY") {
		return String(g_nVPDCorrectionPriority);
		...
		return String(g_cCALLMEBOT_PHONE);
	} else if (var == "SSID") {
...
void setup() {
	...
	if (!SafeSDAccess([&]() {
		...
		if (pSettingsFile) {
			...
			///////////////////////////////////////////////////
			ReadFromStream(pSettingsFile, cBuffer, sizeof(cBuffer));  // CALLME BOT APY KEY

			strncpy(g_cCALLMEBOT_API_KEY, cBuffer, sizeof(g_cCALLMEBOT_API_KEY));
			g_cCALLMEBOT_API_KEY[sizeof(g_cCALLMEBOT_API_KEY) - 1] = '\0';
			///////////////////////////////////////////////////
			ReadFromStream(pSettingsFile, cBuffer, sizeof(cBuffer));  // CALLME BOT PHONE NUMBER
	...
	g_pWebServer.on("/", HTTP_GET, [](AsyncWebServerRequest* pRequest) {
		if (pRequest->hasArg("action")) {  // Process the request
			if (pRequest->arg("action") == "restart") {
				...
				// =============== CallMeBot =============== //
				strncpy(g_cCALLMEBOT_API_KEY, pRequest->arg("apikey").c_str(), sizeof(g_cCALLMEBOT_API_KEY) - 1);
				g_cCALLMEBOT_API_KEY[sizeof(g_cCALLMEBOT_API_KEY) - 1] = '\0';

				strncpy(g_cCALLMEBOT_PHONE, pRequest->arg("phone").c_str(), sizeof(g_cCALLMEBOT_PHONE) - 1);
				g_cCALLMEBOT_PHONE[sizeof(g_cCALLMEBOT_PHONE) - 1] = '\0';
	...
	if ((nCurrentMillis - nLastSecondTick) >= 1000) { // Check if 1 second has passed since the last tick to perform once-per-second tasks
		...
		// ================================================== Irrigation Section ================================================== //
		{
			...
			if (!bApplyIrrigation) { // Si no se está aplicando un Riego; Verificar si se puede aplicar un Riego
				...
				uint8_t nStartIrrigationHour = (g_nEffectiveStartLights + g_nPulseIntervalAfterStart) % 24;
				uint8_t nStopIrrigationHour = (g_nEffectiveStopLights - g_nPulseIntervalBeforeEnd + 24) % 24;
