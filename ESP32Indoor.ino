...
void SafeSDAccess(std::function<void()> fn) {
  ...
  if (!g_bIsSDInit) {
    ...
    if (!g_bIsSDInit) {
#if defined(TEST_MODE) && defined(ENABLE_SERIAL_LOGGER)
        Serial.println("[ERR] Failed to initialize SD.");
#endif
...
#if defined(TEST_MODE) && defined(ENABLE_SERIAL_LOGGER)
    Serial.println("[ERR] Filesystem not accessible or corrupted.");
#endif
...
String HTMLProcessor(const String &var) {
  if (var == "DPM") {
    ...
    for (uint8_t i = 0; i < nSoilHumidityPinsCount; i++)
      strReturn += ",{label:'Humedad de Maceta " + String(i) + "',borderColor:'" + g_strSoilGraphColor[i] + "',backgroundColor:'" + g_strSoilGraphColor[i] + "',symbol:'%%',yAxisID:'3'}";
  ...
  pWebServer.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (request->hasArg("action")) {  // Process the request
      if (request->arg("action") == "reload") {  // This is for reload Elements from the Panel based on selected Profile
        ...
        */
        request->send(200, "text/plain;charset=utf-8", "REFRESH" + strResponse);
        return;
      }
    ...
    request->send(500, "text/plain", "No tengo respuesta para esto.");
  });
...
void loop() {
  ...
  if ((ulCurrentMillis - ulLastSecondTick) >= 1000) { // Check if 1 second has passed since the last tick to perform once-per-second tasks
    ...
    // ================================================== Time Section ================================================== //
    {
      static uint32_t nLastWriteTick = 0;

      if ((ulLastSecondTick - nLastWriteTick) >= 10000 /*10 Seconds*/) {
        nLastWriteTick = ulLastSecondTick;
